...
class User extends Model {
	...
	public static function verifyLogin($type_user = true)
	{

		if (!User::checkLogin($type_user)) {
	...
	public static function countUsers(){
		...
		return $sql->select("SELECT COUNT(*) AS alunos FROM tb_user WHERE type_user = 0");
		...
		else
		{
			...
			else
			{
				...
				$code = base64_encode($code);
					...
					$link = "http://painel.academia.com.br/forgot/reset?code=$code";			
	...
	public function payment($idprofessor, $idclass){

		$sql = new Sql();

		$results = $sql->select("SELECT * FROM tb_pagamento WHERE id_user = :iduser AND id_aluno = :idaluno AND id_class = :idclass",[
			":iduser" => $idprofessor,
			":idaluno" => $this->getid_user(),
			":idclass" => $idclass
		]);
	...
	public function devendoPagamento($idprofessor, $idclass, $pagamento){

		$sql = new Sql();

		if($this->payment($idprofessor, $idclass) == 0){
		$sql->query("INSERT INTO tb_pagamento(id_user, id_class, id_aluno, pagamento) VALUES(:iduser, :idclass, :idaluno, :pagamento)",[
			":iduser" => $idprofessor,
			":idclass" => $idclass,
			":idaluno" => $this->getid_user(),
			":pagamento" => $pagamento
			...
			$sql->query("UPDATE tb_pagamento SET pagamento = :pagamento WHERE id_user = :iduser AND id_class = :idclass AND id_aluno = :idaluno",[
				":iduser" => $idprofessor,
				":idclass" => $idclass,
				":idaluno" => $this->getid_user(),
				":pagamento" => $pagamento
	...
	public static function getLessonPayment($iduser){
		
		$sql = new Sql();

		return $sql->select("SELECT * FROM tb_date_hour_class a 
		INNER JOIN tb_class b ON a.id_class = b.id_class
		INNER JOIN tb_user c ON a.id_user = c.id_user
	...
	public static function getPayment($iduser){
		
		$sql = new Sql();

		return $sql->select("SELECT a.pagamento FROM tb_pagamento a 
		WHERE a.id_aluno = :iduser",[
			":iduser" => $iduser
		]);
